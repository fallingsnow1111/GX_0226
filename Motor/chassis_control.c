...
void chassis_control(void)
{
    ...
    static uint8_t read_div = 0;

    static uint8_t lost_feedback_count = 0; //丢失反馈计数器
    static uint8_t feedback_accum = 0; //跨周期累积反馈标志，适配分频查询
    uint8_t feedback_ready;

    taskENTER_CRITICAL();
    feedback_accum |= motor_check.flag_finish;
    motor_check.flag_finish = 0; // 快照后立即清零，避免与中断置位竞态
    feedback_ready = feedback_accum;
    taskEXIT_CRITICAL();

    if ((feedback_ready & ALL_MOTOR_FEEDBACK_READY) == ALL_MOTOR_FEEDBACK_READY)
    {
        feedback_accum = 0;
